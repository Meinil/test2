...
Page({
  ...
  onConfirmTimerPicker(event) {
    //使用 Vant 提供的时间选择器组件，获取的时间是时间戳
    //需要将时间戳转换成年月日在页面中进行展示才可以
    //可以调用小程序给提供的日期格式化方法对时间戳进行转换
  ...
  async getAddress(){
      ...
      //如果存在数据就需要从全局共享的 address 中取到数据进行赋值
  ...
  async getOrderInfo(){
    ...
    const { data: orderInfo} = goodsId 
     ...
     //判断是否存在祝福语
     //如果需要购买的是多个商品，筛选第一个存在的祝福语的商品进行赋值
  ...
  //在页面加载的时候出发啊
  onLoad(options) {
    //获取立即购买商品传递的参数，然后把参数赋值给 data 中的状态
    this.setData({
      ...options
    })
  },
  ...
  onUnload() {
    //在页面销毁以后，需要将全局共享的 address 也进行销毁
    //如果用户再次进入结算页面支付页面，需要从接口地址默认的收货地址进行渲染
