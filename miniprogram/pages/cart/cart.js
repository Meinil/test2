  ...
  computed:{
    ...
    totalPrice(data){
      ...
      data.cartList.forEach((item) =>{
        //需要判断商品是否是选中的状态， isChecked 是否等于1
        if(item.isChecked === 1) {
  ...
  methods: {
      ...
      //更新购买的数量
       changeBuyNum: debounce(async function(event){
        //获取最新的购买数量
        //如果用户输入的购买数量大于200， 需要把购买的数量设置为200
       ...
       //使用正则验证用户输入的购买数量，是否是 1-200 之间的正整数
        ...
        const resRes = reg.test(newBuyNum)

        //如果验证没有通过，说明用户输入了购买数量不合法或者小于1 ，需要还原之前的购买数量
        if(!resRes){
          this.setData({
        ...
        //如果服务器更新购买数量，需要更新本地的购买数量
        if(res.code === 200){
          this.setData({
            [`cartList[${index}].count`]: newBuyNum,
            //如果购买数量发生了改变，需要让当前的商品变成选中的状态
      ...
      //展示文案的同时获取购物车列表数据
       async showTipGetList() {
        ...
        if(!token) {
          this.setData({
            emptyDes:'你尚未登录，点击登录获取更多权益',
            cartList:[]
          })

          return
        }
        //如果用户进行了登录，就需要获取列表的数据
